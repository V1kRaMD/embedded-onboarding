...
.equ RCC_AHBENR,     RCC_BASE + 0x14     // AHB Peripheral enable register IM GUESSING THIS ONE CHAT
...
my_gpio_init:
    ...
    ldr r5, [r0, #0x00]                  // what register are we modifying to set the mode?
    bic r5, r5, r4                      // clear the 2 bits first
    lsl r6, r2, r3
    orr r5, r5, r6
    str r5, [r0, #0x00]                  // write back to the what register
...
ms:
    movs r1, 48                     // adjust based on system clock frq  [CONSTANT]

cnt:
    add r1, r1, #1                    // count down or count up? which instruction mnemonic supports that?
    bl cnt                           // loop until done, what branch condition do we want?
